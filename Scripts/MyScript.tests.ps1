...
# describes the function script:Get-Foo
Describe 'script:Get-Foo' {
  ...
  Context 'Running without arguments'   {
    ...
    It 'runs without errors' {
      ...
      # scriptblock (braces):
      { script:Get-Foo } | Should Not Throw
    }
    It 'does something' {
      # call function script:Get-Foo and pipe the result to an assertion
      # Example:
      # script:Get-Foo | Should Be 'Expected Output'
      # Hint: 
    ...
    It 'does not return anything'     {
      script:Get-Foo | Should BeNullOrEmpty 
    }
  }
}
